...
class ActorCritic(nn.Module):
    ...
    def __init__(self, state_size=18, action_size=3, hidden=128):
        ...
        # Orthogonal init — standard PPO best-practice
...
class RolloutBuffer:
    """Fixed-size rolling buffer — keeps the last `capacity` transitions."""
...
class PPOAgent:
    ...
    - Rule prior  : fires before neural net → correct decisions on day 1
    - Reward norm : clips rewards to [-3,+3] → stable value learning
    - Online PPO  : trains every step on a rolling 32-step window → fast learning
    - Low LR/c1   : prevents value loss explosion on high-variance rewards
    """
    ...
    def __init__(self, deployment_name, namespace="default",
        ...
        # ── PPO hyper-parameters (tuned for this environment) ──────────────
        self.gamma          = 0.99
        self.gae_lambda     = 0.95
        self.clip_epsilon   = 0.2
        self.c1             = 0.1    # value loss coeff  (was 0.5 → exploded)
        self.c2             = 0.05   # entropy coeff     (encourages exploration)
        self.lr             = 1e-4   # learning rate     (was 3e-4 → too large)
        self.epochs         = 2      # PPO epochs        (was 4 → overfitting)
        self.mini_batch     = 8      # mini-batch size
        self.train_every    = 1      # train after every N steps (online PPO)
        self.buffer_capacity= 32     # rolling window size
        ...
        self.batch_size      = self.buffer_capacity  # alias for app.py logging
    ...
    def select_action(self, state, deterministic=False):
        ...
        Rule prior fires first for obvious cases → confidence is high from
        day 1 without any training.
        """
    ...
    def calculate_reward(self, metrics, action, prev_metrics):
        """Compute raw reward, update normaliser, return raw for logging."""
    ...
    def store_transition(self, state, action, reward, next_state,
        ...
        """Normalise reward then add to rolling buffer."""
    ...
    def update(self):
        """PPO update on current buffer contents."""
        if len(self.buffer) < self.mini_batch:
            return None

        states, actions, rewards, old_log_probs, old_values, dones = \
            self.buffer.get()
    ...
    def pretrain(self, n_steps=300):
        ...
        # Each entry: (cpu, mem, request_rate, latency_p95, replicas)
        # Metric values match REAL Prometheus output for this cluster
        scenarios = [
            (0.02, 0.01, 0.0, 0.80, 1),   # stressed 1 pod  → scale_up
            (0.02, 0.01, 0.0, 0.75, 2),   # stressed 2 pods → scale_up
            (0.02, 0.01, 0.0, 0.70, 3),   # stressed 3 pods → no_action (cap)
            (0.02, 0.01, 0.0, 0.72, 4),   # over-scaled     → no_action
            (0.02, 0.01, 0.0, 0.72, 5),   # over-scaled     → scale_down
            (0.01, 0.01, 0.0, 0.10, 3),   # idle 3 pods     → scale_down
            (0.01, 0.01, 0.0, 0.08, 2),   # idle 2 pods     → scale_down
            (0.02, 0.01, 0.0, 0.30, 1),   # stable 1 pod    → no_action
        ]
        ...
        logger.info(f"✅ PPO pre-training complete — "
                    f"{train_calls} updates over {n_steps} steps")
        return train_calls
    ...
    def load_model(self, filepath):
        ...
        # Support both new key ('policy') and old DQN/PPO key ('policy_net',
        # 'policy_state_dict') so stale .pt files don't cause a crash.
    ...
    def _rule_prior(self, state):
        ...
        if replicas <= 1 and latency > self.sla_target:
            return 2   # single pod under stress → must scale up

        if violation > 0.2 and replicas <= 2:
            return 2   # clearly over SLA with very few pods → scale up

        if latency < self.sla_target * 0.5 and replicas > 1:
            return 0   # clearly idle and over-provisioned → scale down

        return None    # ambiguous → let PPO decide
    ...
    def _raw_reward(self, metrics, action, prev_metrics):
        ...
        # Over-scaling penalty (stops runaway scale-up with vish/stress)
        ...
        if prev_metrics:
            under_stress = latency > self.sla_target
            low_load     = (latency < self.sla_target * 0.6 and
                            replicas > 1)

            if action == 2:   # scale_up
                if under_stress and replicas <= 3:
                    reward += 25.0
                elif replicas > 3:
                    reward -= 10.0
                elif not under_stress:
                    reward -= 15.0
                ...
                if replicas <= 1:
                    ...
                    reward += 20.0
                elif under_stress and replicas <= 3:
                    reward -= 20.0
                elif replicas > 3 and under_stress:
                    reward += 5.0   # scaling back reasonable when over-provisioned

            elif action == 1:  # no_action
                if not under_stress and replicas == 1:
                    reward += 8.0
                elif under_stress and replicas >= 4:
                    reward += 5.0
                elif under_stress and replicas <= 2:
                    reward -= 12.0
                elif low_load:
                    reward -= 8.0
    ...
    def _gae(self, rewards, values, dones):
        ...
        for t in reversed(range(n)):
            ...
            delta     = (rewards[t] + self.gamma * next_val * (1 - dones[t])
                         - values[t])
            advantages[t] = last_adv = (delta + self.gamma * self.gae_lambda
                                        * (1 - dones[t]) * last_adv)
